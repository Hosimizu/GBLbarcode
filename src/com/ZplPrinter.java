...
*/          
    ...
    public ZplPrinter(String printerURI){  
        ...
        //加载字体  
        //File file = new File("C://Users//wan//Desktop//WMS//barcode//ts24.lib");  
        String path = ZplPrinter.class.getResource("") + "ts24.lib";
        ...
        printService = PrintServiceLookup.lookupDefaultPrintService();//默认的PrintService,即默认的打印机设备
    }  
    /** 
     * 设置条形码 
     * @param barcode 条码字符 
        ...
        for (int off = 0; off < ch.length;) {  
            if (((int) ch[off] & 0x00ff) >= 0xA0) {  
                ...
                content += String.format("^FO%d,%d^XG0000%01X%01X,%d,%d^FS\n", new Object[]{new Integer(x), 
                		new Integer(y), new Integer(qcode), new Integer(wcode), new Integer(mx), new Integer(my)}); 
                begin += String.format("~DG0000%02X%02X,00072,003,\n",new Object[]{new Integer(qcode), new Integer(wcode)});  
                ...
                for (int j = 0; j < 72; j += 3) {  
                    ...
                    begin += String.format("%02X%02X%02X\n", new Object[]{new Integer(qcode), new Integer(wcode), new Integer(qcode1)});  
                }  
                x = x + ms * mx;  
                off = off + 2;  
            } else if (((int) ch[off] & 0x00FF) < 0xA0) {  
            	String s = String.format("%c", new Object[]{new Integer(ch[off])});
                setChar(s, x, y, eh, ew);  
                x = x + es;  
                off++;  
            }  
